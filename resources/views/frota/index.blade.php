            ...
            <div
                ...
                <div class="p-6">
                    ...
                    <div class="flex justify-center mt-5 gap-6 border-t pt-3 text-sm font-medium text-gray-700">
                        ...
                        {{-- 🔹 Ação exclusiva para responsáveis --}}
                        @elseif(isset($frota->ehResponsavel) && $frota->ehResponsavel)
                        <form action="{{ route('frota.abandonar', $frota) }}" method="POST"
                            onsubmit="return confirm('Deseja realmente sair desta frota?')" class="inline">
                            @csrf
                            <button type="submit" title="Abandonar responsabilidade desta frota"
                                class="flex items-center text-red-600 hover:text-red-800 transition gap-1">
                                🚪 <span>Sair</span>
                            </button>
                        </form>
