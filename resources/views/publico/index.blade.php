...
<div class="mt-6 text-sm text-gray-700">
    ...
    <div class="flex flex-col items-center gap-3 sm:hidden">
        ...
        <div class="bg-white/40 backdrop-blur-sm px-4 py-2 rounded-lg border border-white/30 shadow-sm text-center">
            Página <strong>{{ $itens->currentPage() }}</strong>
            de <strong>{{ $itens->lastPage() }}</strong> —
        ...
        <div class="w-full flex justify-center overflow-x-auto">
            <div class="inline-flex gap-1 px-2 pb-1">
                ...
                {{-- Primeira página --}}
                @if ($inicio > 1)
                    <a href="{{ $itens->url(1) }}" class="px-3 py-1 rounded-lg border bg-white hover:bg-gray-50">1</a>
                    @if ($inicio > 2)
                ...
                @for ($i = $inicio; $i <= $fim; $i++)
                    @if ($i == $current)
                        <span class="px-3 py-1 rounded-lg border border-blue-500 bg-blue-500 text-white font-semibold">{{ $i }}</span>
                    @else
                        <a href="{{ $itens->url($i) }}" class="px-3 py-1 rounded-lg border bg-white hover:bg-gray-50">{{ $i }}</a>
                    @endif
                @endfor
                ...
                @if ($fim < $total)
                    @if ($fim < $total - 1)
                        <span class="px-3 py-1">…</span>
                    @endif
                    <a href="{{ $itens->url($total) }}" class="px-3 py-1 rounded-lg border bg-white hover:bg-gray-50">{{ $total }}</a>
                @endif

                {{-- Próxima --}}
                @if ($itens->hasMorePages())
                    <a href="{{ $itens->nextPageUrl() }}" class="px-3 py-1 rounded-lg border bg-white hover:bg-gray-50">»</a>
                @else
